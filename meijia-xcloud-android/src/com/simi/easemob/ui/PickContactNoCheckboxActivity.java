...
public class PickContactNoCheckboxActivity extends EMBaseActivity {
...
//		}
	...
	private void getContactList() {
		contactList.clear();
		Map<String, EaseUser> users = EMDemoHelper.getInstance().getContactList();
		Iterator<Entry<String, EaseUser>> iterator = users.entrySet().iterator();
		while (iterator.hasNext()) {
			Entry<String, EaseUser> entry = iterator.next();
			if (!entry.getKey().equals(EMConstant.NEW_FRIENDS_USERNAME) && !entry.getKey().equals(EMConstant.GROUP_USERNAME) && !entry.getKey().equals(EMConstant.CHAT_ROOM) && !entry.getKey().equals(EMConstant.CHAT_ROBOT))
