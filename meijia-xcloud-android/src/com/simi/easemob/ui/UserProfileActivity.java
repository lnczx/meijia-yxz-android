...
public class UserProfileActivity extends EMBaseActivity implements OnClickListener{
	...
	public void asyncFetchUserInfo(String username){
		EMDemoHelper.getInstance().getUserProfileManager().asyncGetUserInfo(username, new EMValueCallBack<EaseUser>() {
			...
			public void onSuccess(EaseUser user) {
				if (user != null) {
					...
					EMDemoHelper.getInstance().saveContact(user);
				}
			}
	...
	private void updateRemoteNick(final String nickName) {
		...
		new Thread(new Runnable() {
			...
			@Override
			public void run() {
				boolean updatenick = EMDemoHelper.getInstance().getUserProfileManager().updateCurrentUserNickName(nickName);
	...
	private void uploadUserAvatar(final byte[] data) {
		...
		new Thread(new Runnable() {
			...
			@Override
			public void run() {
				final String avatarUrl = EMDemoHelper.getInstance().getUserProfileManager().uploadUserAvatar(data);
				runOnUiThread(new Runnable() {
					@Override
					public void run() {
						dialog.dismiss();
