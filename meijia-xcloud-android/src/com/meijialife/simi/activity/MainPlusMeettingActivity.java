...
public class MainPlusMeettingActivity extends BaseActivity implements OnClickListener, ItemScroListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        v= getLayoutInflater()
                .inflate(R.layout.layout_main_plus_meetting, null);
    ...
    private void initView(Cards card) {
        ...
        tv_meeting_location = (EditText) findViewById(R.id.tv_meeting_location);
        tv_huiyi_content = (TextView) findViewById(R.id.tv_huiyi_content);
                ...
                if (on) {
                    if (StringUtils.isEquals(is_senior, "1")) {// 有秘书
                        if (chooseDate == null) {
                            ...
                            UIUtils.showToast(MainPlusMeettingActivity.this, "请选择时间");
                            return;
                        }
                        ...
                        //选择时间不对的情况走else
                            ...
                            UIUtils.showActionDialog(MainPlusMeettingActivity.this, "提醒", "秘书工作时间为7:00～19:00，请在此时间内设置秘书提醒时间，0:01—15:00可以设置当天11:00之后的提醒；15:01至0:00可以设置次日7:00之后的提醒。","确定", null, null, null);
                        }
                        ...
                        tv_senser_tip.setVisibility(View.GONE);
                        SET_SEC_DO = NO_SEC_DO;
                        startActivity(new Intent(MainPlusMeettingActivity.this, FindSecretaryActivity.class));
                        Toast.makeText(MainPlusMeettingActivity.this, "你没有购买秘书卡", Toast.LENGTH_SHORT).show();
                    }
                } else {
                    tv_senser_tip.setVisibility(View.GONE);
                }
        ...
        if (null != card) {
            ...
            Constants.CARD_ADD_MEETING_CONTENT = card.getService_content();
    ...
    private View InitTimeRemind(View view) {
        ...
        arryadapter = new ArrayWheelAdapter<>(this, reminItems);
        remind.setViewAdapter(arryadapter);
        remind.setCyclic(false);// 是否可循环滑动
        remind.setVisibleItems(reminItems.length);// 设置显示行数
    ...
    private void createMeetingCard(boolean update) {
        ...
        if (!isUpdate) {// 如果不是更新的
            if (null != Constants.TEMP_FRIENDS && Constants.TEMP_FRIENDS.size() > 0) {
                ...
                UIUtils.showToast(MainPlusMeettingActivity.this, "请选择参会人员");
                dismissDialog();
                return;
            }
        ...
        if (StringUtils.isEmpty(meetingtime)) {
            UIUtils.showToast(MainPlusMeettingActivity.this, "请选择会议时间");
            dismissDialog();
            return;
        }
        if (StringUtils.isEmpty(location)) {
            UIUtils.showToast(MainPlusMeettingActivity.this, "请选择会议地点");
            dismissDialog();
            return;
        }
        ...
        map.put("service_addr", location);
        map.put("service_content", Constants.CARD_ADD_MEETING_CONTENT);
...
//                            AlermUtils.initAlerm(MainPlusMeettingActivity.this, remindAlerm, fdate, "会议安排", Constants.CARD_ADD_MEETING_CONTENT);
                            ...
                            Toast.makeText(MainPlusMeettingActivity.this, getString(R.string.param_missing), Toast.LENGTH_SHORT).show();
                        } else if (status == Constants.STATUS_PARAM_ILLEGA) { // 参数值非法
                            Toast.makeText(MainPlusMeettingActivity.this, getString(R.string.param_illegal), Toast.LENGTH_SHORT).show();
                        } else if (status == Constants.STATUS_OTHER_ERROR) { // 999其他错误
                            Toast.makeText(MainPlusMeettingActivity.this, msg, Toast.LENGTH_SHORT).show();
                        } else {
                            Toast.makeText(MainPlusMeettingActivity.this, getString(R.string.servers_error), Toast.LENGTH_SHORT).show();
                        }
                    }
                    ...
                    if (StringUtils.isNotEmpty(t.toString())) {
                        ...
                        if (status == Constants.STATUS_SUCCESS) { // 正确
                            if(StringUtils.isNotEmpty(data)){
                                ...
                                TipPopWindow addPopWindow = new TipPopWindow(MainPlusMeettingActivity.this,appHelpData,action);  
