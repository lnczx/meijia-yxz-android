...
public class YearAlermReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        List<AlertCardData> alertList = AssetsDatabaseManager.searchAlertCardByPeriod(db, "5");

        for (Iterator iterator = alertList.iterator(); iterator.hasNext(); ) {
            AlertCardData alertCardData = (AlertCardData) iterator.next();

            Long serviceLong = DateUtils.getLongByPattern(Long.valueOf(alertCardData.getService_time()),"MM-dd HH:mm").getTime();
            Long currentLong = DateUtils.getLongByPattern(System.currentTimeMillis() , "MM-dd HH:mm").getTime();
            boolean flag = serviceLong / 1000 == currentLong / 1000;
            Log.d("tag", "每年提醒--- "+flag);
            Log.d("tag", "每年提醒CurrentStr--- " + currentLong);
            Log.d("tag", "每年提醒serviceStr " + serviceLong);
