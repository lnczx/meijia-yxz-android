...
public class WeekAlermReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        List<AlertCardData> alertList = AssetsDatabaseManager.searchAlertCardByPeriod(db, "3");

        Log.d("tag", "每周提醒数据个数" + alertList.size());

        for (Iterator iterator = alertList.iterator(); iterator.hasNext(); ) {
            AlertCardData alertCardData = (AlertCardData) iterator.next();

            Log.d("tag", "每周提醒实体" + alertCardData);

            try {

                //根据日期判断周
                String currentStr = DateUtils.getStringByPattern(System.currentTimeMillis(), "yyyy-MM-dd");
                String serviceStr = DateUtils.getStringByPattern(Long.valueOf(alertCardData.getService_time()), "yyyy-MM-dd");
                ...
                Log.d("tag", "每周提醒--- " + flag);
                Log.d("tag", "每周提醒CurrentStr--- " + currentLong);
                Log.d("tag", "每周提醒serviceStr " + serviceLong);

                if (CalendarUtils.dayForWeek(serviceStr) == CalendarUtils.dayForWeek(currentStr)) {
                    if (flag) {
                        date = DateUtils.getDateByPattern(alertCardData.getService_time(), "yyyy-MM-dd HH:mm");
                        ...
                        intent2.putExtra("card_id", alertCardData.getAlert_id());
                        intent2.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                        context.startActivity(intent2);
                        AlermUtils.playAudio(context);
                        getAlarm(context,alertCardData.getAlert_id());
                    }
