...
public class DBHelper {
    ...
    public DBHelper(Context context) {
        ...
        finalDb = FinalDb.create(context, DBNAME, false, DBVERSION, new DbUpdateListener() {
     ...
     * @return
     */
    public static <T> T searchById(Class<T> clazz, Object id) {
     ...
     * @return
     */
    public static <T> T searchById(T t, Object id) {
    ...
    public <T> List<T> getCellsList4Db(Class<T> clazz, String cityId) {
        List<T> list = null;
        try {
            String sqlWhere = "C_CITY_ID = " + cityId + " and C_IN_CONTROLL = 1";
     ...
     * @return 数据实体
     */
    public <T> Serializable getDataForValue(Class<T> clazz, String column, String value) {
     ...
     */
    /*
     * public CellsData getCellsData(String c_name){ ArrayList<CellsData> list = new ArrayList<CellsData>(); CellsData cellData; String sqlWhere =
     * "C_NAME="+"'" + c_name + "'"; list = (ArrayList<CellsData>) searchAllByWhere(CellsData.class, sqlWhere); if(list.size() > 0){ cellData =
     * list.get(0); return cellData; }else{ return null; } }
     */
     ...
     * @param alertCard
     */
    public static void addAlertCard(Context context, AlertCardData alertCard) {
        getInstance(context).add(alertCard,alertCard.getAlert_id());
    }
     ...
     * @return
     */
    public static List<AlertCardData> getAlertData(Context context) {
        List<AlertCardData> list = getInstance(context).searchAll(AlertCardData.class);
        if (list.size() > 0) {
            return list;
        }
        return list;
     ...
     */
    /*
     * public AddressDataManager getDefAddressData(){ ArrayList<AddressDataManager> list = new ArrayList<AddressDataManager>(); AddressDataManager
     * addressData; String sqlWhere = "IS_DEFAULT="+"'" + "1" + "'"; list = (ArrayList<AddressDataManager>) searchAllByWhere(AddressDataManager.class,
     * sqlWhere); if(list.size() > 0){ addressData = list.get(0); return addressData; }else{ return null; } }
     */
