...
public class FeedListActivity extends BaseActivity implements OnClickListener, ListItemClickHelpes {
  ...
  public void getAdList() {
    if (!NetworkUtils.isNetworkConnected(FeedListActivity.this)) {
      Toast.makeText(FeedListActivity.this, getString(R.string.net_not_open), Toast.LENGTH_SHORT)
          .show();
      return;
    }
                  ...
                  if (StringUtils.isNotEmpty(t.toString())) {
                    ...
                    if (status == Constants.STATUS_SUCCESS) { // 正确
                      if (StringUtils.isNotEmpty(data)) {
                        Gson gson = new Gson();
                        adBeanList =
                            gson.fromJson(data, new TypeToken<ArrayList<AdData>>() {}.getType());
  ...
  private void getFeedList(int page, String feed_from) {
    ...
    new FinalHttp()
        .get(
            Constants.URL_GET_FRIEND_DYNAMIC_LIST,
            param,
            new AjaxCallBack<Object>() {
              @Override
              public void onFailure(Throwable t, int errorNo, String strMsg) {
                  ...
                  if (StringUtils.isNotEmpty(t.toString())) {
                    ...
                    if (status == Constants.STATUS_SUCCESS) { // 正确
                      if (StringUtils.isNotEmpty(data)) {
                        Gson gson = new Gson();
                        myFeedDataList =
                            gson.fromJson(data, new TypeToken<ArrayList<FeedData>>() {}.getType());
