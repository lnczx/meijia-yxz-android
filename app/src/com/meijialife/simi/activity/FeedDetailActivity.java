...
public class FeedDetailActivity extends BaseActivity
    implements OnClickListener, ListItemClickHelper {
  ...
  private void initCompanyView() {
    ...
    headerView =
        getLayoutInflater().inflate(R.layout.layout_feed_detail, mPullRefreshListView, false);
  ...
  private void getFeedCommentList(int page, String fid) {
    ...
    new FinalHttp()
        .get(
            Constants.URL_GET_DYNAMIC_COMMENT_LIST,
            param,
                  ...
                  if (StringUtils.isNotEmpty(t.toString())) {
                    ...
                    if (status == Constants.STATUS_SUCCESS) { // 正确
                      if (StringUtils.isNotEmpty(data)) {
                        Gson gson = new Gson();
                        myFeedCommentDataList =
                            gson.fromJson(
                                data, new TypeToken<ArrayList<FeedCommentData>>() {}.getType());
  ...
  private void getFeed(String fid) {
    ...
    new FinalHttp()
        .get(
            Constants.URL_GET_DYNAMIC_DETAIL,
            param,
            new AjaxCallBack<Object>() {
              @Override
              public void onFailure(Throwable t, int errorNo, String strMsg) {
  ...
  private void postCloseCommen(String fid) {
    if (user != null) {
      ...
      new FinalHttp()
          .post(
              Constants.URL_POST_CLOSE_COMMENT,
              param,
              new AjaxCallBack<Object>() {
                @Override
                public void onFailure(Throwable t, int errorNo, String strMsg) {
                ...
                public void onSuccess(Object t) {
                  ...
                  try {
                    if (StringUtils.isNotEmpty(t.toString())) {
                      ...
                      String data = obj.getString("data");
                      if (status == Constants.STATUS_SUCCESS) { // 正确
                        Toast.makeText(FeedDetailActivity.this, "关闭问题成功", Toast.LENGTH_SHORT)
                            .show();
                        finish();
                      } else if (status == Constants.STATUS_SERVER_ERROR) { // 服务器错误
                        errorMsg = getString(R.string.servers_error);
