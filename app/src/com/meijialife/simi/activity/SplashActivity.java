...
public class SplashActivity extends Activity implements Runnable {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    //            public void run() {
    //                List<User> searchAll =
    // DBHelper.getInstance(SplashActivity.this).searchAll(User.class);
    //                if (searchAll.size() > 0) {
    ...
    layout_skip_view.setOnClickListener(
        new OnClickListener() {
          @Override
          public void onClick(View v) {
            autoSkip();
  ...
  private void updateUserInfo() {
    ...
    if (!NetworkUtils.isNetworkConnected(SplashActivity.this)) {
      Toast.makeText(SplashActivity.this, getString(R.string.net_not_open), Toast.LENGTH_SHORT)
          .show();
      return;
    }
      ...
      for (int i = 0; i < jsons.length(); i++) {
        ...
        CityData cityData =
            new CityData(
                ...
                name,
                province_id,
                Integer.parseInt(is_enable),
                Long.parseLong(add_time));
  ...
  private void updateDataBase(JSONObject json) {
    ...
    try {
      ...
      int youliaored = baseData.getYouliaoredpoint();
      SpFileUtil.saveInt(
          SplashActivity.this,
          SpFileUtil.FILE_UI_PARAMETER,
          SpFileUtil.KEY_YOULIAO_RED,
          youliaored);
    } catch (JSONException e) {
    }
  }
