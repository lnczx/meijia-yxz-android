...
public class MainActivity extends EMBaseActivity implements OnClickListener, EMEventListener {
  ...
  // =========================以下环信相关内容==========================
  private ImageView unreadImageView, unread_youliao_number; // 未读消息图片
  ...
  private boolean getFristFlag() {
    ...
    boolean FristFlag =
        SpFileUtil.getBoolean(this, SpFileUtil.FILE_UI_PARAMETER, SpFileUtil.KEY_FIRST_INTO, true);
  ...
  private void initIM(Bundle savedInstanceState) {
    if (savedInstanceState != null
        && savedInstanceState.getBoolean(EMConstant.ACCOUNT_REMOVED, false)) {
    ...
    if (getIntent().getBooleanExtra(EMConstant.ACCOUNT_CONFLICT, false) && !isConflictDialogShow) {
      showConflictDialog();
    } else if (getIntent().getBooleanExtra(EMConstant.ACCOUNT_REMOVED, false)
  ...
  private void init() {
    ...
    unread_youliao_number = (ImageView) findViewById(R.id.unread_youliao_number);
  ...
  //        defDrawable = (BitmapDrawable)
  // getResources().getDrawable(R.drawable.ic_defult_touxiang);
  //
      ...
      case R.id.item_6: // 积分商城
        ...
        intent6.putExtra(
            ...
            Constants.URL_POST_SCORE_SHOP
                + "?user_id="
                + DBHelper.getUserInfo(MainActivity.this).getUser_id()); // 配置自动登陆地址，每次需服务端动态生成。
  ...
  private void change(Fragment fragment) {
    if (null == mFM) mFM = getSupportFragmentManager();
  ...
  protected void onResume() {
    ...
    int numberState =
        SpFileUtil.getInt(
            MainActivity.this, SpFileUtil.FILE_UI_PARAMETER, SpFileUtil.KEY_YOULIAO_RED, 0);
    if (numberState == 0) {
      unread_youliao_number.setVisibility(View.GONE);
    } else if (numberState == 1) {
      unread_youliao_number.setVisibility(View.VISIBLE);
    }
  }
  ...
  protected void onStart() {
    ...
    boolean flag =
        SpFileUtil.getBoolean(
            MainActivity.this, SpFileUtil.KEY_MSG_UNREAD, SpFileUtil.KEY_MSG_UNREAD, false);
    if (flag) {
  ...
  protected void onNewIntent(Intent intent) {
    ...
    if (intent.getBooleanExtra(EMConstant.ACCOUNT_CONFLICT, false) && !isConflictDialogShow) {
      showConflictDialog();
    } else if (intent.getBooleanExtra(EMConstant.ACCOUNT_REMOVED, false)
        && !isAccountRemovedDialogShow) {
      showAccountRemovedDialog();
    }
  }
                ...
                MainActivity.this.runOnUiThread(
                    new Runnable() {
                      ...
                      public void run() {
                        ...
                        dismissDialog();
                        Toast.makeText(
                                MainActivity.this, "unbind devicetokens failed", Toast.LENGTH_SHORT)
                            .show();
   ...
   *
   * @param date 卡片服务时间
   * @param remindAlerm 提醒设置 0 = 不提醒 1 = 按时提醒 2 = 5分钟 3 = 15分钟 4 = 提前30分钟 5 = 提前一个小时 6 = 提前2小时 7 =
   *     提前6小时 8 = 提前一天 9 = 提前两天
   * @return
   */
